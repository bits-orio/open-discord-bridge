...
This project ships two things under one version: the **Go bridge** (GitHub release binaries + GHCR images) and the **companion Factorio mod** (`companion-mod/`, optionally the mod portal). `companion-mod/info.json`'s `version` is the single source of truth, and the release tag must be `v<that version>` (the release workflow fails the build if they disagree).

## Steps

1. **Bump the version in `companion-mod/info.json`** if not already done. **Default to a patch bump** (e.g. `0.4.4` → `0.4.5`). Only do a minor or major bump if the user explicitly asks for one — do not infer from the diff.
...
2. **Verify user-facing docs** are still accurate for the changes since the last release. Check these surfaces:
   - `README.md` (repo root, if present) and `bridge/README.md` — external-facing descriptions for GitHub readers / bridge operators.
   - `companion-mod/README.md` — the integrator API and mod install notes (also what mod portal viewers see).
   - `DEPLOYMENT.md` / `TESTING.md` — deployment models, env vars, verification steps.
...
4. **Recreate the companion-mod symlinks** by running:
   ```bash
   ./companion-mod/link-mod.sh
   ```
...
6. **Release** (when the user asks): push the bump commit, then run `./tools/release.sh`. The script verifies the changelog entry, creates and pushes `v<new_version>`, and the GitHub Actions workflow takes over (build companion-mod zip + cross-platform binaries → GitHub release → GHCR images → Discord → mod portal upload).
   ...
   - Required secrets on the GitHub repo (all optional): `FACTORIO_API_KEY` (scope: ModPortal: Upload Mods), `DISCORD_WEBHOOK` (changelog channel), `DISCORD_ANNOUNCEMENTS_WEBHOOK` (announcements channel).
   - **First mod-portal release only:** the Factorio upload API adds a *release* to an *existing* mod page — create the `open-discord-bridge` mod page once on mods.factorio.com before the portal upload can succeed.
